...
export default function DeviceHeader({
  ...
  const INPUT_OPTIONS = [
    { label: 'TV', key: 'InputTuner' },
    { label: 'HDMI 1', key: 'InputHDMI1' },
    { label: 'HDMI 2', key: 'InputHDMI2' },
    { label: 'HDMI 3', key: 'InputHDMI3' },
    { label: 'HDMI 4', key: 'InputHDMI4' }
  ];
  ...
  return (
    <header className="bg-slate-900/90 backdrop-blur border-b border-slate-800 px-3 sm:px-4 py-2 sticky top-0 z-30 shadow-md">
      <div className="max-w-[1700px] mx-auto flex items-center justify-between gap-2 sm:gap-4">
        ...
        {/* Left: Device Selector + Power Button + Inputs Switcher */}
        <div className="flex items-center gap-2">
          ...
          {/* Top Bar Inputs (Desktop/Tablet Segmented Pills) */}
          <div className="hidden md:flex items-center bg-slate-950 p-1 rounded-xl border border-slate-800 gap-1">
            {INPUT_OPTIONS.map((inp) => (
              <button
                key={inp.key}
                onClick={() => handleKey(inp.key)}
                disabled={disabled}
                className="px-2.5 py-1 rounded-lg text-xs font-semibold text-slate-400 hover:text-slate-100 hover:bg-slate-800/80 transition-all active:scale-95 disabled:opacity-40"
              >
                {inp.label}
              </button>
            ))}
          ...
          {/* Mobile/Compact Inputs Dropdown */}
          <div className="relative md:hidden">
            <button
              onClick={() => setInputDropdownOpen(!inputDropdownOpen)}
              disabled={disabled}
              className="flex items-center gap-1 px-2.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-xs font-semibold text-slate-300 transition-all active:scale-95 disabled:opacity-40"
            >
              <span>Input</span>
            ...
            {inputDropdownOpen && (
              <>
                ...
                <div className="absolute left-0 top-full mt-1.5 w-36 bg-slate-900 border border-slate-700 rounded-xl shadow-xl p-1.5 z-50 text-xs space-y-1">
                  {INPUT_OPTIONS.map((inp) => (
                    <button
                      key={inp.key}
                      onClick={() => {
                      ...
                      className="w-full text-left px-2.5 py-1.5 rounded-lg text-slate-200 hover:bg-purple-600/30 hover:text-purple-300 transition-colors font-medium"
                    >
                      {inp.label}
                    </button>
                  ))}
